...
/// @title Beefy Passive Position Manager. Version: Algebra
/// @author weso, Beefy
/// @notice This is a contract for managing a passive concentrated liquidity position on Algebra.
contract StrategyPassiveManagerAlgebra is StratFeeManagerInitializable, IStrategyConcLiq, IStrategyUniswapV3 {
    ...
    /// @notice The max and min ticks allowed.
    ...
    function initialize (
        ...
        pool = _pool;
        quoter = _quoter;
        lpToken0 = IAlgebraPool(_pool).token0();
        lpToken1 = IAlgebraPool(_pool).token1();
    ...
    function _addLiquidity() private {
        ...
        if (liquidity > 0 && amountsOk) {
            minting = true;
            IAlgebraPool(pool).mint(address(this), address(this), positionMain.tickLower, positionMain.tickUpper, liquidity, "Beefy Main");
        }
        ...
        if (liquidity > 0) {
            minting = true;
            IAlgebraPool(pool).mint(address(this), address(this), positionAlt.tickLower, positionAlt.tickUpper, liquidity, "Beefy Alt");
        }
    }
    ...
    function _removeLiquidity() private {
        ...
        (uint128 liquidity,,,,,) = IAlgebraPool(pool).positions(keyMain);
        (uint128 liquidityAlt,,,,,) = IAlgebraPool(pool).positions(keyAlt);

        // If we have liquidity in the positions we remove it and collect our tokens.
        if (liquidity > 0) {
            IAlgebraPool(pool).burn(positionMain.tickLower, positionMain.tickUpper, liquidity);
            IAlgebraPool(pool).collect(address(this), positionMain.tickLower, positionMain.tickUpper, type(uint128).max, type(uint128).max);
        }

        if (liquidityAlt > 0) {
            IAlgebraPool(pool).burn(positionAlt.tickLower, positionAlt.tickUpper, liquidityAlt);
            IAlgebraPool(pool).collect(address(this), positionAlt.tickLower, positionAlt.tickUpper, type(uint128).max, type(uint128).max);
        }
    }
    ...
    function _claimEarnings() private returns (uint256 fee0, uint256 fee1, uint256 feeAlt0, uint256 feeAlt1) {
        ...
        if (liquidity > 0) IAlgebraPool(pool).burn(positionMain.tickLower, positionMain.tickUpper, 0);
        if (liquidityAlt > 0) IAlgebraPool(pool).burn(positionAlt.tickLower, positionAlt.tickUpper, 0);

        // Collect fees from the pool. 
        (fee0, fee1) = IAlgebraPool(pool).collect(address(this), positionMain.tickLower, positionMain.tickUpper, type(uint128).max, type(uint128).max);
        (feeAlt0, feeAlt1) = IAlgebraPool(pool).collect(address(this), positionAlt.tickLower, positionAlt.tickUpper, type(uint128).max, type(uint128).max);
    ...
    */
    function balancesOfPool() public view returns (uint256 token0Bal, uint256 token1Bal, uint256 mainAmount0, uint256 mainAmount1, uint256 altAmount0, uint256 altAmount1) {
        ...
        (uint128 liquidity,,,,uint256 owed0, uint256 owed1) = IAlgebraPool(pool).positions(keyMain);
        (uint128 altLiquidity,,,,uint256 altOwed0, uint256 altOwed1) = IAlgebraPool(pool).positions(keyAlt);
    ...
    */
    function getKeys() public view returns (bytes32 keyMain, bytes32 keyAlt) {
        keyMain = _getPositionKey(positionMain.tickLower, positionMain.tickUpper);
        keyAlt = _getPositionKey(positionAlt.tickLower, positionAlt.tickUpper);
    }
     ...
     * @param _tickLower The lower tick of the position.
     * @param _tickUpper The upper tick of the position.
     * @return key The key for the position.
    */
    function _getPositionKey(int24 _tickLower, int24 _tickUpper) private view returns (bytes32) {
        address thisAddress = address(this);
        bytes32 key;
        assembly {
            key := or(shl(24, or(shl(24, thisAddress), and(_tickLower, 0xFFFFFF))), and(_tickUpper, 0xFFFFFF))
        }
        return key;
    }
    ...
    */
    function swapFee() external override view returns (uint256 fee) {
        (,,uint16 _fee,,,,) = IAlgebraPool(pool).globalState();
        fee = uint256(_fee) * SQRT_PRECISION / 1e6;
    }
    ...
    */
    function setLpToken0ToNativePath(bytes calldata _path) public onlyOwner {
        if (_path.length > 0) {
            (address[] memory _route) = AlgebraUtils.pathToRoute(_path);
    ...
    */
    function lpToken0ToNative() external view returns (address[] memory) {
        ...
        return AlgebraUtils.pathToRoute(lpToken0ToNativePath);
    }
    ...
    */
    function lpToken1ToNative() external view returns (address[] memory) {
        ...
        return AlgebraUtils.pathToRoute(lpToken1ToNativePath);
    }
    ...
    */
    function twap() public view returns (int56 twapTick) {
        ...
        (int56[] memory tickCuml,,,) = IAlgebraPool(pool).getTimepoints(secondsAgo);
